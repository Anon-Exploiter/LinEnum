...
{ 
echo -e "\n#########################################################" 
echo -e "#" "Local Linux Enumeration & Privilege Escalation Script" "#"
echo -e "#########################################################"
echo -e "# www.rebootuser.com | @rebootuser "
echo -e "# $version\n"
echo -e "# Example: ./LinEnum.sh -k keyword -r report -e /tmp/ -t \n"
...
if [ "$sudopass" ]; then 
  echo -e "[+] Please enter password - INSECURE - really only for CTF use!"
  read -s userpassword
  echo 
fi
...
if [[ ! -z $adm_users ]];
  then
    echo -e "[-] It looks like we have some admin users:\n$adm_users"
    echo -e "\n"
fi
...
if [ "$sudoers" ]; then
  echo -e "[-] Sudoers configuration (condensed):$sudoers"
  echo -e "\n"
fi
...
if [ "$sudopass" ]; then
    if [ "$sudoperms" ]; then
      ...
      if [ "$sudoauth" ]; then
        echo -e "[+] We can sudo when supplying a password!\n$sudoauth" 
        echo -e "\n"
      fi
    fi
fi
...
if [ "$thorough" = "1" ]; then
  ...
  if [ "$grfilesall" ]; then
    echo -e "[-] Files not owned by user but writable by group:\n$grfilesall" 
    echo -e "\n"
  fi
fi
...
if [ "$thorough" = "1" ]; then
  ...
  if [ "$ourfilesall" ]; then
    echo -e "[-] Files owned by our user:\n$ourfilesall"
    echo -e "\n"
  fi
fi
...
if [ "$sshrootlogin" = "yes" ]; then
  echo -e "[-] Root is allowed to login via SSH:" ; grep "PermitRootLogin " /etc/ssh/sshd_config 2>/dev/null | grep -v "#" 
  echo -e "\n"
fi
}
...
if [ "$sestatus" ]; then
  echo -e "[-] SELinux seems to be present:\n$sestatus"
  echo -e "\n"
fi
...
if [ "$umaskdef" ]; then
  echo -e "[-] umask value as specified in /etc/login.defs:\n$umaskdef" 
  echo -e "\n"
fi
...
if [ "$logindefs" ]; then
  echo -e "[-] Password and storage information:\n$logindefs" 
  echo -e "\n"
fi
...
if [ "$htpasswd" ]; then
    echo -e "[-] htpasswd found - could contain passwords:\n$htpasswd"
    echo -e "\n"
fi
...
if [ "$intsgid" ]; then
  echo -e "[+] Possibly interesting SGID files:\n$intsgid" 
  echo -e "\n"
fi
...
if [ "$wwsgid" ]; then
  echo -e "[+] World-writable SGID files:\n$wwsgid" 
  echo -e "\n"
fi
...
if [ "$wwsgidrt" ]; then
  echo -e "[+] World-writable SGID files owned by root:\n$wwsgidrt" 
  echo -e "\n"
fi
...
if [ "$fileswithcaps" ]; then
  echo -e "[+] Files with POSIX capabilities set:\n$fileswithcaps"
  echo -e "\n"
fi
...
if [ "$userswithcaps" ]; then
  echo -e "[+] Users with specific POSIX capabilities:\n$userswithcaps"
  echo -e "\n"
fi
...
matchedcaps=`echo -e "$userswithcaps" | grep \`whoami\` | awk '{print $1}' 2>/dev/null`
	if [ "$matchedcaps" ]; then
		echo -e "[+] Capabilities associated with the current user:\n$matchedcaps"
		echo -e "\n"
		...
		if [ "$matchedfiles" ]; then
			echo -e "[+] Files with the same capabilities associated with the current user (You may want to try abusing those capabilties):\n$matchedfiles"
			echo -e "\n"
			...
			echo -e "[+] Permissions of files with the same capabilities associated with the current user:\n$matchedfilesperms"
			echo -e "\n"
			if [ "$matchedfilesperms" ]; then
				...
				if [ "$writablematchedfiles" ]; then
					echo -e "[+] User/Group writable files with the same capabilities associated with the current user:\n$writablematchedfiles"
					echo -e "\n"
				fi
			fi
		fi
...
gitcredfiles=`find / -name ".git-credentials" 2>/dev/null`
	if [ "$gitcredfiles" ]; then
  		echo -e "[+] Git credentials saved on the machine!:\n$gitcredfiles"
  		echo -e "\n"
	fi
fi
...
wwfiles=`find / ! -path "*/proc/*" ! -path "/sys/*" -perm -2 -type f -exec ls -la {} 2>/dev/null \;`
	if [ "$wwfiles" ]; then
		echo -e "[-] World-writable files (excluding /proc and /sys):\n$wwfiles" 
		echo -e "\n"
	fi
fi
...
if [ "$rhostssys" ]; then
  echo -e "[+] Hosts.equiv file and contents: \n$rhostssys" 
  echo -e "\n"
fi
...
if [ "$keyword" = "" ]; then
  echo -e "[-] Can't search *.php files as no keyword was entered\n" 
    ...
    if [ "$phpkey" ]; then
      echo -e "[-] Find keyword ($keyword) in .php files (recursive 10 levels - output format filepath:identified line number where keyword appears):\n$phpkey" 
      echo -e "\n" 
     else 
  echo -e "[-] Find keyword ($keyword) in .php files (recursive 10 levels):" 
...
if [ "$keyword" = "" ];then
  echo -e "[-] Can't search *.log files as no keyword was entered\n" 
    ...
    if [ "$logkey" ]; then
      echo -e "[-] Find keyword ($keyword) in .log files (recursive 4 levels - output format filepath:identified line number where keyword appears):\n$logkey" 
      echo -e "\n" 
     else 
	echo -e "[-] Find keyword ($keyword) in .log files (recursive 4 levels):" 
...
if [ "$keyword" = "" ];then
  echo -e "[-] Can't search *.ini files as no keyword was entered\n" 
    ...
    if [ "$inikey" ]; then
      echo -e "[-] Find keyword ($keyword) in .ini files (recursive 4 levels - output format filepath:identified line number where keyword appears):\n$inikey" 
      echo -e "\n" 
     else 
	echo -e "[-] Find keyword ($keyword) in .ini files (recursive 4 levels):" 
...
if [ "$bakfiles" ]; then
  echo -e "[-] Location and Permissions (if accessible) of .bak file(s):"
...
if [ "$dockercontainer" ]; then
  echo -e "[+] Looks like we're in a Docker container:\n$dockercontainer" 
  echo -e "\n"
fi
...
if [ "$dockerhost" ]; then
  echo -e "[+] Looks like we're hosting Docker:\n$dockerhost" 
  echo -e "\n"
fi
...
if [ "$dockergrp" ]; then
  echo -e "[+] We're a member of the (docker) group - could possibly misuse these rights!\n$dockergrp" 
  echo -e "\n"
fi
...
if [ "$dockerfiles" ]; then
  echo -e "[-] Anything juicy in the Dockerfile:\n$dockerfiles" 
  echo -e "\n"
fi
...
if [ "$dockeryml" ]; then
  echo -e "[-] Anything juicy in docker-compose.yml:\n$dockeryml" 
  echo -e "\n"
fi
}
...
if [ "$lxccontainer" ]; then
  echo -e "[+] Looks like we're in a lxc container:\n$lxccontainer"
  echo -e "\n"
fi
...
if [ "$lxdgroup" ]; then
  echo -e "[+] We're a member of the (lxd) group - could possibly misuse these rights!\n$lxdgroup"
  echo -e "\n"
fi
}
